  ...
  try {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (value && value !== "all" && key !== "role") {
        // Skip role filter when getting role distribution
    ...
    if (Object.keys(filters).length > 0) {
      ...
      if (filteredUserIds && filteredUserIds.length > 0) {
        userPlansQuery = userPlansQuery.in(
          "user_id",
          filteredUserIds.map((u) => u.id)
        );
      }
    }
    ...
    let registrationQuery = supabase.from("profiles").select("created_at");
...
export const fetchQuestionnaireStatistics = async (
  dateRange = null,
  filters = {}
) => {
  try {
    ...
    const { count: completedQuestionnaires } = await completedQuery
      .count()
      .single();

    // Calculate completion rate
    const completionRate =
      totalResponses > 0 ? (completedQuestionnaires / totalResponses) * 100 : 0;
...
export const fetchSubscriptionStatistics = async (
  ...
  try {
    ...
    const { count: totalSubscriptions } = await subscriptionQuery
      .count()
      .single();
    ...
    const monthlyRevenue = subscriptionsByPlan.reduce(
...
export const fetchQuestionAnswerStats = async (
  ...
  try {
    ...
    if (Object.keys(filters).length > 0) {
      ...
      Object.entries(filters).forEach(([key, value]) => {
        if (value && value !== "all") {
          responsesQuery = responsesQuery.in(
            "user_id",
            filteredUserIds.map((u) => u.id)
          );
        }
...
export const fetchDemographicData = async (
  ...
  try {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (value && value !== "all" && key !== field) {
        // Skip filter on the target field
